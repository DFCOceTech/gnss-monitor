...
> Version: 1.2 | Status: Active | Last updated: 2026-05-09
...
[ZED-X20P via USB]
        ...
        ↓
[GNSSCollector thread]  ── pyubx2 UBXReader ──→  NAV-PVT / NAV-SAT / MON-RF
        |
        | on_sample() callback (1 Hz)
        ↓
[app.py main process]
    ├── Storage (SQLite WAL)
    │      ├── gnss_samples (1 Hz)
    │      ├── satellite_metrics (per SV)
    │      ├── rf_metrics (per band)
    ...
    │
    ├── AnomalyDetector
    │      ├── Threshold checks (jammingState, spoofDetState, jamInd)
...
1. Send raw UBX poll frames for NAV-PVT, NAV-SAT, NAV-STATUS, MON-RF (20 ms apart) — **do NOT reset input buffer** first, to preserve any auto-output bytes already queued
...
8. If `_current_pvt` is set: `_emit()` bundles pvt + sat + rf → calls `on_sample()`

### Sample Processing
1. Write to SQLite (gnss_samples, satellite_metrics, rf_metrics)
2. Baseline recomputation check
3. AnomalyDetector.process() → insert events if thresholds exceeded
4. Update in-memory state dict + history ring buffer (120 samples)
...
- **Decision**: Each cycle polls all four messages (NAV-PVT, NAV-SAT, NAV-STATUS, MON-RF) explicitly without resetting the input buffer, collects all bytes for 1.1 s (covers NAV-PVT epoch latency), then emits one sample with all data bundled.
- **Consequences**: All four message types are always fresh per cycle. Auto-output bytes (NAV-SAT, NAV-STATUS at ~0.33 Hz) are also captured. Cycle runs at ~0.9 Hz due to 1.1 s window dominating.
